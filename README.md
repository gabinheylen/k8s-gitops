...
kubectl delete secret vaultwarden-admin-auth -n vaultwarden
kubectl create secret generic vaultwarden-admin-auth -n vaultwarden \
  --from-literal=users="$(echo 'admin:HASH_GENERE' | base64 -w 0)"

# 3. Mettre à jour le fichier Git
# Éditer vaultwarden/admin-auth-secret.yaml avec le nouveau hash
git add vaultwarden/admin-auth-secret.yaml
git commit -m "Update admin BasicAuth password"
git push origin main
```
...
kubectl patch secret vaultwarden-admin-token -n vaultwarden \
  -p='{"data":{"admin-token":"'$(echo 'NOUVEAU_TOKEN' | base64 -w 0)'"}}'

# 3. Redémarrer Vaultwarden
